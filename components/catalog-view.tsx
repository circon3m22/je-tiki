...
export function CatalogView() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!active) return;
      if (!error && data) {
        setCatalogProducts(data.map((row) => mapSupabaseProduct(row, products.find((item) => item.slug === row.slug))));
      } else if (error) {
        setLoadFailed(true);
      }
      setRefreshing(false);
  ...
  return (
    <>
      ...
      <p className="sr-only" role="status">
        {refreshing ? "Обновляем каталог. " : ""}Показано украшений: {filtered.length}.
        {loadFailed ? " Не удалось обновить данные — показан сохранённый каталог." : ""}
      </p>
      {filtered.length === 0 ? (
